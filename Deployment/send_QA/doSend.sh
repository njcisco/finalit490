...
/usr/bin/sshpass -p "${PASS_M}" ssh -t "${USER_M}"@"${IP_D}" 'cp -r  ~/QA/Files/*  ~/QA/Backup'
...
/usr/bin/sshpass -p "${PASS_M}" ssh -t "${USER_M}"@"${IP_D}" 'cp -r   /var/www/fly490-qa.com/*  /var/www/Backup'
